...
def create_custom_audio_list_random_split(
    csv_filepath: str,
    audio_base_dir: str,
    output_list_dir: str,
    test_size: float = 0.2, # 测试集比例，默认为 0.2 (20%)
    random_state: int = 42 # 随机种子，用于复现性
):
    """
    根据给定的CSV元数据和音频目录，随机划分生成训练集、测试集和标签列表文件。
    ...
    for index, row in df.iterrows():
        ...
        if not os.path.exists(sound_path):
            # print(f"警告: 音频文件 '{sound_path}' 不存在，跳过此文件。")
        ...
        data_for_split.append({
            ...
            'class_id': class_id,
            'label_name': label_name # 保留标签名称，如果需要进行分层抽样
        })
    ...
    # 将列表转换为DataFrame以便进行随机划分
    data_df_for_split = pd.DataFrame(data_for_split)

    # 随机划分训练集和测试集
    # stratify=data_df_for_split['label_name'] 用于分层抽样，确保训练集和测试集中的类别分布相似
    train_data, test_data = train_test_split(
        data_df_for_split,
        test_size=test_size,
        random_state=random_state,
        stratify=data_df_for_split['label_name'] # 强烈推荐使用分层抽样，特别是类别不平衡时
    )
...
if __name__ == "__main__":
    ...
    my_output_list_dir = "/home/renmengxing/audioRec/processed_data/lists_random_split/" # 新的输出目录，避免覆盖

    create_custom_audio_list_random_split(
        csv_filepath=my_csv_file,
        audio_base_dir=my_audio_dir,
        output_list_dir=my_output_list_dir,
        test_size=0.2, # 80% 训练，20% 测试
        random_state=42 # 保持一致的划分结果
    )